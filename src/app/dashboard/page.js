...
import { Loader, Lock, GraduationCap, Users, Briefcase, Landmark } from "lucide-react";
import { useRouter } from "next/navigation";

export default function DashboardRouter() {
  const { user, profile, loading, loginWithGoogle, saveUserProfile } = useAuth();
  const [selectedRole, setSelectedRole] = useState("");
  const [roleLoading, setRoleLoading] = useState(false);
  const [error, setError] = useState("");
  const router = useRouter();

  // Redirect staff accounts to the admin console automatically
  useEffect(() => {
      ...
      <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        ...
        <main style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 20px" }}>
          <div className="glass-card" style={{ maxWidth: "480px", width: "100%", padding: "40px", textAlign: "center" }}>
            <div style={{ background: "rgba(79, 70, 229, 0.1)", color: "var(--primary)", padding: "16px", borderRadius: "50%", display: "inline-flex", marginBottom: "20px" }}>
              <Lock size={32} />
            </div>
            <h2 style={{ fontSize: "1.75rem", marginBottom: "12px", fontFamily: "var(--font-display)" }}>Login Required</h2>
            <p style={{ color: "var(--text-secondary)", lineHeight: "1.6", marginBottom: "32px" }}>
              Please sign in with Google to access your dashboard timeline, link your institutions, and apply for verified placements.
            </p>
            <button 
              onClick={loginWithGoogle} 
              className="btn-primary" 
              style={{ width: "100%", padding: "14px", display: "inline-flex", gap: "10px" }}
            >
              <svg width="20" height="20" viewBox="0 0 24 24">
                <path fill="#FFFFFF" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                <path fill="#FFFFFF" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fillOpacity="0.85" />
                <path fill="#FFFFFF" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l3.66-2.85z" fillOpacity="0.85" />
                <path fill="#FFFFFF" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.85c.87-2.6 3.3-4.53 6.16-4.53z" fillOpacity="0.85" />
              </svg>
